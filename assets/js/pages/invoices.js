async function renderInvoices(){
  ...
  target.innerHTML = `
    ...
    <div class="mt-3 overflow-x-auto bg-white border rounded-xl">
      <table class="table text-sm" id="tbl">
        <thead><tr>
          <th>N°</th><th>Date</th><th>Client</th><th>Statut</th><th>Total</th><th></th>
        </tr></thead>
        <tbody></tbody>
      </table>
    </div>
  ...
  target.querySelector('#add').onclick = ()=> openInvoiceForm();
  tbody.addEventListener('click', async (e)=>{
    const id = Number(e.target.getAttribute('data-id'));
    if(e.target.classList.contains('view')) openInvoiceView(id);
    if(e.target.classList.contains('edit')) openInvoiceForm(state.invoices.find(x=>x.id===id));
    if(e.target.classList.contains('delete')){
      const inv = state.invoices.find(x=>x.id===id);
      if(inv.status==='paid') return alert('Impossible de supprimer une facture payée.');
  ...
  function openInvoiceForm(inv=null){
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-2xl p-4 w-[980px] max-w-[98vw] max-h-[95vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-2">
          <h3 class="font-semibold">${isEdit?'Modifier':'Nouvelle'} facture</h3>
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <select id="status" class="border rounded p-2 w-full">
              <option value="pending" ${data.status!=='paid'?'selected':''}>En attente</option>
              <option value="paid" ${data.status==='paid'?'selected':''}>Payée</option>
            </select>
          </div>
          <div class="col-span-3">
            <label class="text-xs text-gray-500">Notes (visible sur la facture)</label>
    ...
    modal.querySelector('#save').onclick = async ()=>{
      ...
      modal.remove(); renderInvoices();
    };
  }

  function openInvoiceView(id){
    const inv = state.invoices.find(x=>x.id===id);
    const client = state.clients.find(x=>x.id===inv.clientId) || {};
    const settings = state.settings;
    const totalHT = (inv.items||[]).reduce((a,it)=>{
      const line = (it.qty||1)*(it.unitPriceHT||0);
      const disc = it.discountType==='percent' ? line*(it.discountValue||0)/100 :
                   it.discountType==='amount' ? (it.discountValue||0) : 0;
      return a + (line - disc);
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-2xl p-4 w-[980px] max-w-[98vw] max-h-[95vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-2">
          <h3 class="font-semibold">Facture ${inv.number}</h3>
          <div class="flex gap-2">
            <button class="px-3 py-2 border rounded dl">Télécharger PDF</button>
            <button class="p-2 border rounded close"><i class="lucide-x"></i></button>
          </div>
        </div>
        <div id="invoice" class="border rounded-2xl p-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <div class="font-medium">Facturé à</div>
            ...
            <div class="text-right">
              <div>Facture n° <strong>${inv.number}</strong></div>
              <div>Date: <strong>${inv.date}</strong></div>
              <div>Statut: <strong>${inv.status==='paid'?'Payée':'En attente'}</strong></div>
            </div>
          </div>
          ...
          <div class="mt-4 flex flex-col items-end gap-1">
            ...
            ${inv.notes? `<div class="text-sm mt-2"><strong>Notes:</strong> ${inv.notes}</div>` : ''}
          </div>
        </div>
      </div>
    `;
    ...
    modal.querySelector('.dl').onclick = async ()=>{
      ...
      const el = modal.querySelector('#invoice');
